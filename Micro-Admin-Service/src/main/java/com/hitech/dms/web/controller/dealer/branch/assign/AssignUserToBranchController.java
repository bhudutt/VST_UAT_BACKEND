 ...
 * 
 */
package com.hitech.dms.web.controller.dealer.branch.assign;
...
import com.hitech.dms.web.dao.dealer.assign.branch.AssignUserToBranchDao;
...
public class AssignUserToBranchController {
	...
	@Autowired
	private AssignUserToBranchDao assignUserToBranchDao;

	private SimpleDateFormat getSimpleDateFormat() {
		return new SimpleDateFormat("dd-M-yyyy hh:mm:ss");
	}

	@PostMapping("/assignBranchesToUser")
	public ResponseEntity<?> assignBranchesToUser(@Valid @RequestBody AssignUserToBranchFormModel requestedData,
		...
		try {
			responseModel = assignUserToBranchDao.assignBranchesToUser(userCode, requestedData);
			if (responseModel.getStatusCode() == 201) {
				codeResponse.setCode("EC200");
				codeResponse.setDescription("Branch Assignment on " + formatter.format(new Date()));
				codeResponse.setMessage(responseModel.getMsg());
			} else if (responseModel.getStatusCode() == 500) {
				codeResponse.setCode("EC500");
				codeResponse.setDescription("Unsuccessful on " + formatter.format(new Date()));
				codeResponse.setMessage("Branch To User Not Assigned or server side error.");
			}
		} catch (Exception e) {
