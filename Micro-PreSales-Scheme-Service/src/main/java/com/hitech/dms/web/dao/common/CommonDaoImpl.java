...
public class CommonDaoImpl implements CommonDao {
	...
	public List<SchemeTypeMstResponseModel> fetchSchemeTypeMstList(String userCode, String isIncludeInactive) {
		Session session = null;
		List<SchemeTypeMstResponseModel> responseList = null;
		try {
			session = sessionFactory.openSession();
			responseList = fetchSchemeTypeMstList(session, userCode, isIncludeInactive);
	...
	public List<SchemeTypeMstResponseModel> fetchSchemeTypeMstList(Session session, String userCode,
			String isIncludeInactive) {
		Query query = null;
		List<SchemeTypeMstResponseModel> responseList = null;
		String sqlQuery = "exec [SP_SA_SCHEME_TYPE_LIST] :userCode, :isIncludeInactive";
		try {
			...
			if (data != null && !data.isEmpty()) {
				responseList = new ArrayList<SchemeTypeMstResponseModel>();
				for (Object object : data) {
					Map row = (Map) object;
					SchemeTypeMstResponseModel responseModel = new SchemeTypeMstResponseModel();
					responseModel.setSchemeTypeId((Integer) row.get("SchemeTypeId"));
					responseModel.setSchemeTypeCode((String) row.get("SchemeTypeCode"));
					responseModel.setSchemeTypeName((String) row.get("SchemeTypeName"));
					responseModel.setIncentiveUnit((String) row.get("IncentiveUnit"));
					responseModel.setDisplayName((String) row.get("DisplayValue"));
