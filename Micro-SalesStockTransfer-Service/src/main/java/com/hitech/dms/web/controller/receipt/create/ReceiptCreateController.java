 ...
 * 
 */
package com.hitech.dms.web.controller.receipt.create;
...
import com.hitech.dms.web.dao.receipt.create.ReceiptCreateDao;
...
public class ReceiptCreateController {
	...
	@Autowired
	private ReceiptCreateDao createDao;
	...
	public ResponseEntity<?> create(@RequestHeader(value = "Authorization", required = true) String authorizationHeader,
			@RequestBody @Valid ReceiptCreateRequestModel requestModel, OAuth2Authentication authentication,
		...
		ReceiptCreateResponseModel responseModel = createDao.createReceipt(authorizationHeader, userCode, requestModel,
				device);
		if (responseModel.getStatusCode() == 201) {
			codeResponse.setCode("EC200");
			codeResponse.setDescription("Receipt created on " + formatter.format(new Date()));
			...
			codeResponse.setMessage("Receipt Not Created Or Server Side Error.");
