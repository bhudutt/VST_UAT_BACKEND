...
public interface PdiRepository extends JpaRepository<PdiEntity, Long> {
	...
	@Query(value = "exec sp_service_pdi_search_autocomplete_chassis_number :searchString,:userCode", nativeQuery = true)
    List<Map<String, Object>> searchAutocompleteChassisNumber(@Param("searchString") String searchString, @Param("userCode") String userCode);
	 ...
	 @Query(value = "exec sp_service_pdi_search :pdiFromDate,:pdiToDate,:chassisNo,:engineNo,:dmsGrnFromDate,:dmsGrnToDate,:pdiNumber,:page,:size,:invoiceNumber,:userCode", nativeQuery = true)
