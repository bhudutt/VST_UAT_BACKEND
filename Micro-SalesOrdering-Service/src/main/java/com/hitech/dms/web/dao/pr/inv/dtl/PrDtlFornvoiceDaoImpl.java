...
import com.hitech.dms.web.model.pr.inv.dtl.response.PrItemDtlViewResponseModel;
import com.hitech.dms.web.model.pr.inv.dtl.response.PrMachDtlViewResponseModel;
 ...
 */
@Repository
public class PrDtlFornvoiceDaoImpl implements PrDtlFornvoiceDao {
	private static final Logger logger = LoggerFactory.getLogger(PrDtlFornvoiceDaoImpl.class);
	...
	public PrDtlFornvoiceResponseModel fetchPurchaseReturnDtlForInv(String userCode,
		...
		PrDtlFornvoiceResponseModel responseModel = null;
		try {
			session = sessionFactory.openSession();
			requestModel.setFlag(1);
			List data = fetchPurchaseReturnDtlForInv(session, userCode, requestModel);
			if (data != null && !data.isEmpty()) {
				for (Object object : data) {
					Map row = (Map) object;
					responseModel = new PrDtlFornvoiceResponseModel();
				...
				data = fetchPurchaseReturnDtlForInv(session, userCode, requestModel);
				if (data != null && !data.isEmpty()) {
					List<PrMachDtlViewResponseModel> salesMachineGrnDtlList = new ArrayList<PrMachDtlViewResponseModel>();
					for (Object object : data) {
						Map row = (Map) object;
						PrMachDtlViewResponseModel dtlForMachineResponseModel = new PrMachDtlViewResponseModel();
						...
						dtlForMachineResponseModel.setCgstPer((BigDecimal) row.get("CgstPer"));
						dtlForMachineResponseModel.setSgstPer((BigDecimal) row.get("SgstPer"));
						dtlForMachineResponseModel.setIgstPer((BigDecimal) row.get("IgstPer"));
						dtlForMachineResponseModel.setCgstAmnt((BigDecimal) row.get("CgstAmnt"));
						dtlForMachineResponseModel.setSgstAmnt((BigDecimal) row.get("SgstAmnt"));
						dtlForMachineResponseModel.setIgstAmnt((BigDecimal) row.get("IgstAmnt"));
				...
				data = fetchPurchaseReturnDtlForInv(session, userCode, requestModel);
				if (data != null && !data.isEmpty()) {
					List<PrItemDtlViewResponseModel> salesMachineGrnImplDtlList = new ArrayList<PrItemDtlViewResponseModel>();
					for (Object object : data) {
						Map row = (Map) object;
						PrItemDtlViewResponseModel dtlForItemResponseModel = new PrItemDtlViewResponseModel();
						...
						dtlForItemResponseModel.setCgstPer((BigDecimal) row.get("CgstPer"));
						dtlForItemResponseModel.setSgstPer((BigDecimal) row.get("SgstPer"));
						dtlForItemResponseModel.setIgstPer((BigDecimal) row.get("IgstPer"));
						dtlForItemResponseModel.setCgstAmnt((BigDecimal) row.get("CgstAmnt"));
						dtlForItemResponseModel.setSgstAmnt((BigDecimal) row.get("SgstAmnt"));
						dtlForItemResponseModel.setIgstAmnt((BigDecimal) row.get("IgstAmnt"));
	...
	public List fetchPurchaseReturnDtlForInv(Session session, String userCode,
			PrDtlFornvoiceRequestModel requestModel) {
		if (logger.isDebugEnabled()) {
		...
		String sqlQuery = "[SP_SA_GRN_PUR_RET_INV_DTL] :userCode, :pcId, :dealerId, :prId, :prNo, :flag";
		try {
