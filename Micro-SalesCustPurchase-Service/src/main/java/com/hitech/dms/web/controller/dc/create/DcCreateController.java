 ...
 * 
 */
package com.hitech.dms.web.controller.dc.create;
...
import com.hitech.dms.web.dao.dc.create.DcCreateDao;
...
import com.hitech.dms.web.service.validator.dc.create.DcCreateValidator;
...
public class DcCreateController {
	private static final Logger logger = LoggerFactory.getLogger(DcCreateController.class);

	@Autowired
	private DcCreateValidator bindingResultValidator;
	...
	public ResponseEntity<?> create(@RequestHeader(value = "Authorization", required = true) String authorizationHeader,
		...
		DcCreateResponseModel responseModel = null;
		if (bindingResult.hasErrors()) {
			codeResponse.setCode("EC400");
			codeResponse.setDescription("Unsuccessful on " + formatter.format(new Date()));
			codeResponse.setMessage("DC Cancel not created or server side error.");
		...
		responseModel = createDao.createDc(authorizationHeader, userCode, requestModel);	
		if (responseModel != null && responseModel.getStatusCode().compareTo(WebConstants.STATUS_CREATED_201) == 0) {
			...
			codeResponse.setDescription("Unsuccessful on " + formatter.format(new Date()));
			codeResponse.setMessage("Error While Creating DC.");
		}
		userAuthResponse.setResponseCode(codeResponse);
