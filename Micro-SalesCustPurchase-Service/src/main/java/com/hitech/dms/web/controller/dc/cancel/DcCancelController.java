 ...
 * 
 */
package com.hitech.dms.web.controller.dc.cancel;
...
import com.hitech.dms.web.dao.dc.cancel.DcCancelDao;
...
public class DcCancelController {
	private static final Logger logger = LoggerFactory.getLogger(DcCancelController.class);

	@Autowired
	private DcCancelValidator bindingResultValidator;
	...
	public ResponseEntity<?> create(@RequestHeader(value = "Authorization", required = true) String authorizationHeader,
		...
		DcCancelResponseModel responseModel = null;
		...
		responseModel = cancelDao.cancelDc(authorizationHeader, userCode, requestModel);
		if (responseModel != null && responseModel.getStatusCode() != null
			...
			codeResponse.setDescription("Unsuccessful on " + formatter.format(new Date()));
			codeResponse.setMessage("Error While Cancelling DC Cancel.");
		}
		userAuthResponse.setResponseCode(codeResponse);
