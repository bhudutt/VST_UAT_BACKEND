 ...
 * 
 */
package com.hitech.dms.web.controller.inv.create;
...
import com.hitech.dms.web.dao.inv.create.InvoiceCreateDao;
...
import com.hitech.dms.web.service.validator.invoice.create.InvoiceCreateValidator;
...
public class InvoiceCreateController {
	private static final Logger logger = LoggerFactory.getLogger(InvoiceCreateController.class);

	@Autowired
	private InvoiceCreateValidator bindingResultValidator;
	...
	@Autowired
	private InvoiceCreateDao createDao;

	@PostMapping(value = "/create")
	public ResponseEntity<?> create(@RequestHeader(value = "Authorization", required = true) String authorizationHeader,
			@RequestBody InvoiceCreateRequestModel requestModel, BindingResult bindingResult,
		...
		InvoiceCreateResponseModel responseModel = null;
		if (bindingResult.hasErrors()) {
			codeResponse.setCode("EC400");
			codeResponse.setDescription("Unsuccessful on " + formatter.format(new Date()));
			codeResponse.setMessage("Machine Invoice Number not created or server side error.");
		...
		responseModel = createDao.createInvoice(authorizationHeader, userCode, requestModel);
		if (responseModel != null && responseModel.getStatusCode().compareTo(WebConstants.STATUS_CREATED_201) == 0) {
			...
			codeResponse.setDescription("Unsuccessful on " + formatter.format(new Date()));
			codeResponse.setMessage("Error While Creating Machine Invoice.");
			userAuthResponse.setResponseCode(codeResponse);
			userAuthResponse.setResponseData(responseModel);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(userAuthResponse);
		}
