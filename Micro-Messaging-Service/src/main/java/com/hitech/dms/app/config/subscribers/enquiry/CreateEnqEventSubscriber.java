 ...
 *
 */
public class CreateEnqEventSubscriber extends StringToObjectModel {
	private Logger logger = LoggerFactory.getLogger(CreateEnqEventSubscriber.class);
	...
	public CreateEnqEventSubscriber(MsgLogService msgLogService, SmsMailDao smsMailDao, MailUtil mailUtil, CreateMailTemplateModel templateModel, MailDao mailDao) {
	...
	public void receive(String msg) {
		...
		if (publishModel != null) {
			logger.info("Create Enquiry received message '{}'", publishModel.toString());
	...
	private void updateMsgLogData(MailWithTemplate mail, int msgLogStatus, SmsMailRequestModel requestModel,
		...
		MsgLog msgLog = new MsgLog(msgId.toString(), mail, RabbitConfig.mail_enq_create_exchange,
				RabbitConfig.mail_enq_create_routing_key);
