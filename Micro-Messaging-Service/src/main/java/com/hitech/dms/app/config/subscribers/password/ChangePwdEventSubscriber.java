 ...
 *
 */
public class ChangePwdEventSubscriber extends StringToObjectModel {
	private Logger logger = LoggerFactory.getLogger(ChangePwdEventSubscriber.class);
	...
	public ChangePwdEventSubscriber(MsgLogService msgLogService, SmsMailDao smsMailDao, MailUtil mailUtil, CreateMailTemplateModel templateModel, MailDao mailDao) {
	...
	public void receive(String msg) {
		...
		if (publishModel != null) {
			logger.info("Change Password received message '{}'", publishModel.toString());
	...
	private void updateMsgLogData(MailWithTemplate mail, int msgLogStatus, SmsMailRequestModel requestModel,
		...
		MsgLog msgLog = new MsgLog(msgId.toString(), mail, RabbitConfig.mail_change_password_exchange,
				RabbitConfig.mail_change_password_routing_key);
