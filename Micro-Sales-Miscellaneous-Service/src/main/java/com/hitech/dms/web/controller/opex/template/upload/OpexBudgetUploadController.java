 ...
 * 
 */
package com.hitech.dms.web.controller.opex.template.upload;
...
import com.hitech.dms.web.dao.opex.template.upload.OpexBudgetUploadDao;
...
import com.hitech.dms.web.validator.opex.upload.OpexBudgetUploadValidator;
...
public class OpexBudgetUploadController {
	private static final Logger logger = LoggerFactory.getLogger(OpexBudgetUploadController.class);

	@Autowired
	private OpexBudgetUploadValidator bindingResultValidator;
	...
	@Autowired
	private OpexBudgetUploadDao dao;

	private SimpleDateFormat getSimpleDateFormat() {
		return new SimpleDateFormat("yyyy-MM-dd_HH:mm:ss");
	}

	@PostMapping(value = "/uploadOpexBudget")
	public ResponseEntity<?> validateUploadedFile(
			@RequestHeader(value = "Authorization", required = true) String authorizationHeader,
			@RequestPart OpexBudgetUploadRequestModel requestModel,
		...
		OpexBudgetUploadResponseModel responseModel = null;
		if (isFileExist) {
			if (bindingResult.hasErrors()) {
				codeResponse.setCode("EC400");
				codeResponse.setDescription("Unsuccessful on " + formatter.format(new Date()));
				codeResponse.setMessage("Opex Budget Not Added or server side error.");
			...
			responseModel = dao.validateUploadedFile(authorizationHeader, userCode, requestModel, files);
			userAuthResponse.setResponseData(responseModel);
			if (responseModel != null && responseModel.getStatusCode() == WebConstants.STATUS_CREATED_201) {
				...
				codeResponse.setDescription("Unsuccessful on " + formatter.format(new Date()));
				codeResponse.setMessage("Error While Uploading Opex Budget .xlsx file.");
			...
			codeResponse.setCode("EC400");
			codeResponse.setDescription("Unsuccessful on " + formatter.format(new Date()));
			codeResponse.setMessage("Please Upload Opex Budget .xlsx file.");
