...
public class CommonDaoImpl implements CommonDao {
	...
	public GeoStateDTLResponseModel fetchStateDtlByStateID(String authorizationHeader, BigInteger stateId) {
		...
		try {
			HeaderResponse headerResponse = commonServiceClient.fetchStateDtlByStateID(authorizationHeader,
					stateId);
	...
	public List<GlMstResponseModel> fetchGlMstList(Session session, String userCode, Integer pcId) {
		Query query = null;
		List<GlMstResponseModel> responseList = null;
		String sqlQuery = "Select * from SA_MST_GL_HEAD (nolock) GM where pc_id =:pcId";
		try {
			...
			if (data != null && !data.isEmpty()) {
				responseList = new ArrayList<GlMstResponseModel>();
				for (Object object : data) {
					Map row = (Map) object;
					GlMstResponseModel responseModel = new GlMstResponseModel();
					responseModel.setGlId((BigInteger) row.get("gl_id"));
					responseModel.setPcId((Integer) row.get("pc_id"));
					responseModel.setGlHeaderCode((String) row.get("gl_head_code"));
					responseModel.setGlHeaderName((String) row.get("gl_head_name"));
